...
void init_rotary_interrupt() {
	...
	GPIO_PORTA_IS_R &= ~(0x20); // if cleared = detect falling/rising edges, otherwise low/high levels
	GPIO_PORTA_IBE_R |= 0x20; // if cleared = can adjust if interrupted on falling or rising edges
	...
	// Check table 2-9 on the datasheet for the interrupt table, there you can see that the interrupt number for PORTA is 0
	NVIC_PRI7_R |= (3 << 21); // PDF-page 151 in datasheet; setting interrupt priority to 3
	NVIC_EN0_R |= (1 << 0); // shifts 00000....1 (32 bits in total) 0 times to the left,
...
void rotary_interrupt_handler() {
	...
	GPIO_PORTA_ICR_R |= Clear_interrupt; // Clears any previous interrupts on PA5

	if ((GPIO_PORTA_DATA_R & 0x20) && (GPIO_PORTA_DATA_R & 0x40)) { // checks if PA5 and PA6 have the same state
		direction = 'L';
	} else {
		direction = 'R';
	}
