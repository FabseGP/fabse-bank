...
void init_sw1_interrupt() {
    ...
    GPIO_PORTF_IS_R &= ~(0x10); // if cleared = detect falling/rising edges,
                                // otherwise low/high levels
    GPIO_PORTF_IBE_R &=
        ~(0x10); // if cleared = can adjust if interrupted on falling or rising
    ...
    GPIO_PORTF_ICR_R |=
        Clear_interrupt; // clears any previous interrupts on pin PF4
    ...
    NVIC_EN0_R |=
        (1 << 30); // shifts 00000....1 (32 bits in total) 30 times to the left,
