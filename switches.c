...
enum Sw1_debouncer {
	Debounce_time = 50, Clear_interrupt = 0x10, Clear = 0
...
void init_sw1_interrupt() {
	...
	GPIO_PORTF_IS_R &= ~(0x10); // if cleared = detect falling/rising edges, otherwise low/high levels
	GPIO_PORTF_IBE_R &= ~(0x10); // if cleared = can adjust if interrupted on falling or rising edges
	...
	NVIC_EN0_R |= (1 << 30); // shifts 00000....1 (32 bits in total) 30 times to the left,
							 // setting the 30th bit for enabling PORTF interrupt
