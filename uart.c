...
uint32_t lcrh_databits(uint8_t data_size) {
	...
	return (((uint32_t) data_size - 5) << 5); // Control bit 5-6, WLEN
}
...
void uart0_init(uint32_t baud_rate, uint8_t data_size, uint8_t stop_bits,
	...
	GPIO_PORTA_AFSEL_R |= 0x00000003; // Set PA0 and PA1 to alternativ function (uart0)
...
void uart0_task(void *pvParameters) {
	...
	uart0_init(9600, 8, 1, 'n'); // 9600 baudrate, 8 data-bits, 1 stop-bit and no parity
